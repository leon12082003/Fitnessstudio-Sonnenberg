...
from config import CALENDAR_ID, WORK_HOURS
import pytz
...
def book_appointment(dt, name):
    if not is_slot_free(dt):
        return False
    service = get_service()
    tz = pytz.timezone("Europe/Berlin")
    dt = tz.localize(dt)
...
def delete_appointment(dt, name):
    service = get_service()
    tz = pytz.timezone("Europe/Berlin")
    dt = tz.localize(dt)
    end_dt = dt + timedelta(minutes=30)
...
def get_free_slots_for_day(date_str, after_time=None):
    try:
        ...
        start_time, end_time = WORK_HOURS[weekday]
        ...
        while start_dt < end_dt:
            if is_slot_free(start_dt):
                free_slots.append(start_dt.strftime("%H:%M"))
            start_dt += timedelta(minutes=30)
        return free_slots
    except Exception as e:
        print(f"Fehler in get_free_slots_for_day: {e}")
        return []
    ...
    try:
        ...
        for i in range(14):
            try:
                ...
                slots_today = get_free_slots_for_day(date_str, after_time)
                for slot in slots_today:
                    slots.append(f"{date_str} {slot}")
                    if len(slots) == count:
                        return slots
            except Exception as inner_e:
                print(f"Fehler bei Tag {i}: {inner_e}")
                continue
        return slots
    except Exception as e:
        print(f"Fehler in get_next_free_slots: {e}")
        return []
